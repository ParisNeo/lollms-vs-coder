...
user = menu.run()  # Type "al" → filters to "Alice Anderson"
```
...
def restart_service():
    ...
    # Restart logic
    ASCIIColors.green("✓ Service restarted")
...
main.add_submenu("Settings →", settings_menu)  # ↗ indicates submenu
...
menu = Menu(
    ...
    selected_icon="●",              # Selected checkbox
    unselected_icon="○",            # Unselected checkbox
...
launcher = Menu("🚀 Dev Tools", mode='execute', clear_screen_on_run=True)

launcher.add_action("▶  Start Dev Server", run_dev_server)
launcher.add_action("🧪 Run Tests", run_tests)
launcher.add_action("📊 View Logs", open_logs)
launcher.add_separator()  # Visual divider
launcher.add_action("🚀 Deploy to Staging", deploy_staging)
...
def run_setup_wizard():
    ...
    if Menu("Save this configuration?", mode='return').add_choice("✓ Save", True).add_choice("✗ Cancel", False).run():
        save_config(config)
        ASCIIColors.green("✓ Configuration saved!")
...
def build_main_menu():
    ...
    main.add_submenu("File →", file_menu)
    main.add_submenu("Edit →", edit_menu)
    main.add_submenu("Tools →", tools_menu)
