...
export const researchWebPageTool: ToolDefinition = {
    ...
    description: "Scrapes a web page, extracts information relevant to a specific objective using AI, and saves the findings to the project research notes. Returns the extracted content for immediate use by the agent.",
    isAgentic: true,
    isDefault: true,
...
${truncatedText}`;
            ...
            if (env.workspaceRoot) {
                ...
                try { existingContent = await vscode.workspace.fs.readFile(notesUri); } catch (e) { }
            ...
            // CRITICAL FIX: Return the extracted info so it goes into Agent/Architect context
            return { 
                success: true, 
