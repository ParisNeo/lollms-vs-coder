---
name: lollms-instantiation-llamacpp
description: >
...
`llama_cpp_server` is a **self-managed local binding** that runs [llama.cpp](https://github.com/ggerganov/llama.cpp) directly within your Python process. Unlike other bindings, it **does not require an external server** — the binding automatically starts, manages, and stops the llama.cpp server for you.
...
| ✅ Use `llama_cpp_server` when... | ❌ Consider alternatives when... |
|-----------------------------------|--------------------------------|
...
# Absolute minimal — only model_name required
lc = LollmsClient(
...
|-----------|------|---------|------|-------------|
| `model_name` | `str` | `""` | 🔴 | GGUF filename. For split models, use first part only |
| `models_path` | `str` | `"models/llama_cpp_models"` | 🟢 | Directory scanned for `.gguf` files |
| `host` | `str` | `"localhost"` | 🟢 | Server bind address |
| `port` | `int` | `9624` | 🟢 | Server port (file-locked, prevents duplicates) |
| `ctx_size` | `int` | `128000` | 🟢 | Context window in tokens |
| `n_gpu_layers` | `int` | `-1` | 🟢 | GPU offload: `-1`=all, `0`=CPU, `N`=specific layers |
| `n_threads` | `int\|null` | `null` | 🟢 | CPU threads (`null`=auto-detect) |
| `n_parallel` | `int` | `1` | 🟢 | Parallel sequence decoding (concurrent requests) |
...
# Split model — specify FIRST part only, rest auto-detected
...
└── llama_cpp_models/
    ...
    └── mmproj-llava-v1.5-7b-f16.gguf  # Multimodal projector
...
### `pull_model()` — Download from Hugging Face
...
|-----------|------|----------|-------------|
| `repo_id` | `str` | ✅ | HuggingFace repository (`owner/repo-name`) |
| `filename` | `str` | ✅ | Specific GGUF file to download |
| `mmproj_repo_id` | `str` | ❌ | Projector repository (for vision models) |
| `mmproj_filename` | `str` | ❌ | Projector filename |

**Progress Callback**: Returns `{"status": str, "completed": int, "total": int}`

---

### `bind_multimodal_model()` — Manual Projector Association
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `model_name` | `str` | ✅ | Base GGUF model filename |
| `mmproj_name` | `str` | ✅ | Projector GGUF filename |
