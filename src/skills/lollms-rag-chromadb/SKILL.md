...
├── 📁 clients/
│   ├── 📄 contract.docx
│   └── 📄 meeting_notes.msg (Outlook)
└── 📁 presentations/
    └── 📄 pitch.pptx

→ Ingestion Pipeline → Structured Chunks → Vector DB → LoLLMs Agent
```
            ...
            if file_path.is_file() and file_path.suffix.lower() in include_patterns:
                try:
                    ...
                    if not doc.get("error"):
                        ...
                        print(f"✓ Loaded: {file_path.relative_to(root)}")
...
def build_knowledge_base(document_root: str, 
    ...
    print(f"\n✅ Knowledge base ready: {len(all_chunks)} chunks indexed")
...
def ask_agent(question: str, tools: dict, 
    ...
    if "tool_calls" in metadata:
        print(f"\n🔧 Tools used: {len(metadata['tool_calls'])} calls")
