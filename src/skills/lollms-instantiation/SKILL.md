...
|-------|------|-------------|
| `model_name` | `str` | **Always required** — model identifier |
...
| `open_router` | ✅ | Access models from multiple APIs |
...
| Do | Don't |
|--|-------|
| ✅ Add `.env` to `.gitignore` | ❌ Commit keys to version control |
| ✅ Use `load_dotenv()` at startup | ❌ Hardcode keys in source |
| ✅ Set fallback defaults for local dev | ❌ Expose production keys in logs |
| ✅ Rotate keys regularly | ❌ Share keys across projects |
| ✅ Use dedicated keys per environment | ❌ Use personal keys for production |
...
> 📚 **For binding-specific details** (commands, multimodal, advanced params), load the dedicated skill: `python/lollms_client/setup/BINDING_NAME`
