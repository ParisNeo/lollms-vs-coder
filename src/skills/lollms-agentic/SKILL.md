...
Modern LoLLMs agentic system provides **composable answers**, **dynamic scratchpads**, **self-correction**, and **recursive reasoning** — far beyond simple tool calling.
...
| **Unified Tools** | RAG, MCPs, custom functions — one interface |
...
1. search_web("renewable energy breakthroughs 2024")
   → append_to_answer("Theme 1: Renewable Energy...", sources=[...])
   
2. search_web("AI healthcare applications 2024")
   → append_to_answer("Theme 2: AI in Healthcare...", sources=[...])
   
3. search_web("space exploration milestones 2024")
   → append_to_answer("Theme 3: Space Exploration...", sources=[...])
   
4. final_answer() → Combines all sections
```
...
1. Agent assumes: "Fusion commercial by 2030"
   → update_scratchpad("fusion_timeline", "2030", "assumptions")

2. Search reveals: "Most estimates now say 2040+"
   → update_assumption_status("fusion_timeline", "WRONG", 
                              "ITER delays pushed timeline")

3. Agent revises answer:
   → update_answer_section("theme_1", corrected_content, 
...
```python
# Large document — RLM activates automatically
