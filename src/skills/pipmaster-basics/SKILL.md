...
PipMaster is a **programmatic Python package manager** that automates dependency handling directly from your code. No more `pip install -r requirements.txt` — your application manages its own environment.
...
All operations are **idempotent** — safe to run repeatedly, only acts when needed.
...
> 📚 **For advanced features** (virtual environments, uv backend, portable Python, custom index servers), load: `python/pipmaster/advanced`
